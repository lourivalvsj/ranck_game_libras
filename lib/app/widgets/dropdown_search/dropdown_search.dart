...
import 'package:ranck_game_libras/app/widgets/dropdown_search/src/properties/selection_list_view_props.dart';
...
class DropdownSearchState<T> extends State<DropdownSearch<T>> {
  ...
  Color butFocusColor = Colors.black;
  Color butFocusColorDisabled = Colors.white;
  double butFocusSize = 15;
  ...
  Widget _defaultSelectedItemWidget() {
    ...
    Widget selectedItemWidget() {
      ...
      return TextBody(_selectedItemAsString(getSelectedItem));
    }
  ...
  InputDecoration _manageDropdownDecoration(FormFieldState state) {
    return (widget.dropdownSearchDecoration ?? const InputDecoration())
  ...
  Widget _manageTrailingIcons() {
    ...
    dropdownButtonPressed() => widget.loadDropdown; //_selectSearchMode();
    ...
    return Row(
      ...
      children: <Widget>[
        if (getSelectedItems.isNotEmpty &&
          ...
          ExcludeFocus(
            ...
            child: Tooltip(
              ...
              child: InkWell(
                onTap: clearButtonPressed,
                child: widget.clearButton ?? const Icon(Icons.clear, size: 10),
        ...
        //       )
        //     :
        if (getSelectedItems.isEmpty && widget.enabled)
          IgnorePointer(
            ignoring: widget.loadDropdown == null,
            child: Tooltip(
              message: 'Recarregar listagem',
              child: InkWell(
                onTap: widget.loadDropdown,
                child: widget.dropDownButton ??
                    Icon(
                      Icons.replay_rounded,
                      size: butFocusSize,
                      color: widget.loadDropdown != null
                          ? butFocusColor
                          : butFocusColorDisabled,
        ...
        items: [
          CustomPopupMenuItem(
            child: SizedBox(
              ...
              child: _selectDialogInstance(defaultHeight: heightDropdown),
            ),
          ),
        ]);
  }
  ...
  void _handleOnChangeSelectedItems(List<T> selectedItems) {
    ...
    _handleFocus(false);
    if (widget.autoClear && selectedItems.isNotEmpty) {
      clear();
    }
  }
